...
func NewLogListener(a *provision.Box, filterLog provision.Boxlog) (*LogListener, error) {
...
func LogRemove(a *provision.Box) error {
	/* conn, err := db.LogConn()
	if err != nil {
		return err
	}
...
func LogReceiver() (chan<- *provision.Boxlog, <-chan error) {
	/*ch := make(chan *Boxlog)
