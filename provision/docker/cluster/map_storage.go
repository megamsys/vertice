...
/*
func (s *MapStorage) StoreIPIndex() error {
	s.ipMut.Lock()
	defer s.ipMut.Unlock()
	if s.ipMap == nil {
		s.ipMap = make(map[string]uint)
	}
	s.ipMap[containerID] = hostID
	return nil
	}

}

func (s *MapStorage) RetrieveIPIndex(subnet net.IPNet) (uint, error) {
	s.ipMut.Lock()
	defer s.ipMut.Unlock()
	indices := make([]IPIndex, 0, len(s.ipMap))
	for _, ind := range s.ipMap {
		indices = append(indices, *ind)
	}

}

func (s*MapStorage) GetIPIndex(subnet net.IPNet) (net.IPNet, uint, error) {
